...
pub fn rogue_prove<R: Rng>(ck: &CommitKey, instance: &Instance, witness: &Witness, rng: &mut R) -> (Fr, Proof) {
    let a = witness.a;
    debug_assert_eq!(
        ck.commit_with_explicit_randomness(a, witness.r_1),
        instance.comm_1
    ...
    let r = Fr::rand(rng);
    let r_rogue = Fr::rand(rng);
    let (comm_rho, rho) = ck.commit_with_rng(r, rng);
    let (comm_tau, tau) = ck.commit_with_rng(r_rogue, rng);
    let commitment = ProofCommitment {
        comm_rho,
        comm_tau,
    };
    ...
    // Computing bad a2
    let a2 = (r - r_rogue + challenge*a) / challenge;
